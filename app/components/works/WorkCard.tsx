...
import {
  WORK_CARD_DIR_MAP,
  WORK_CARD_ORIGIN_MAP,
  WORK_CARD_TYPE_MAP,
  WORK_CARD_VIEWPORT,
  WORK_CARD_TRANSITION,
} from '@/lib/constants';
...
function WorkCard({
  ...
  const direction =
    WORK_CARD_DIR_MAP[dir as keyof typeof WORK_CARD_DIR_MAP] || 0;
  const typeClasses =
    WORK_CARD_TYPE_MAP[type as keyof typeof WORK_CARD_TYPE_MAP] ||
    WORK_CARD_TYPE_MAP.short;
  const originClass =
    WORK_CARD_ORIGIN_MAP[dir as keyof typeof WORK_CARD_ORIGIN_MAP] ||
    WORK_CARD_ORIGIN_MAP.center;

  const isPriority = index < 2;
  const dimensions =
    type === 'long' ? { width: 324, height: 488 } : { width: 324, height: 236 };
